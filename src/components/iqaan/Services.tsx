...
import type { AccentText } from '@/i18n/dictionaries';
...
export default function Services() {
  ...
  const heading = t.services.heading as AccentText;
        ...
        <div className="mt-16 md:mt-20">
          {t.services.items.map((service, i) => (
            <motion.a
              key={service.title}
              href="#contact"
              aria-label={t.services.rowAria(service.title)}
              onClick={(e) => {
                e.preventDefault();
                document
            ...
            >
              ...
              <span className="hidden items-center justify-end md:flex lg:col-start-4">
                <ArrowRight
                  aria-hidden="true"
                  className="slide-x h-5 w-5 text-ink/30 transition-[transform,color] duration-500 group-hover:text-viridian"
                />
              </span>
            </motion.a>
          ))}
